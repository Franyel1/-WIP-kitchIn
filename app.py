...
def household(household_id):
    ...
    return render_template(
        "household.html",
        grocery_list=household.get("grocery_list", []),
        pantry=household.get("pantry", []),
        current_user_id=str(current_user.id),
        household_id=household_id,
        household_name=household.get("name", "Unnamed Household"),
    )
...
def update_pantry():
    ...
    if result.modified_count > 0:
        ...
        flash("No changes made or item not found.", "warning")

    return redirect(request.referrer or url_for("home"))


@app.route("/update-grocery", methods=["POST"])
@login_required
def update_grocery():
    item_id = request.form.get("id")
    name = request.form.get("name")
    ...
    result = db.households.update_one(
        {"grocery_list._id": ObjectId(item_id)},
        {"$set": {
            "grocery_list.$.name": name,
            "grocery_list.$.price": price,
            "grocery_list.$.requester_id": requester
        }}
    )
